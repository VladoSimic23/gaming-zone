...
export default function ReservationForm() {
      ...
      try {
        ...
        const slots = hours.map((hour: string) => {
          const bookedInSlot = existingReservations
            ...
            .filter((r: ReservationDoc) => r.timeSlots && r.timeSlots.includes(hour))
            .reduce((sum: number, r: ReservationDoc) => sum + (r.seats || 0), 0);
  ...
  const minAvailableInSelected = selectedTimes.length > 0
    ? Math.min(...availableSlots.filter(s => selectedTimes.includes(s.time)).map(s => s.availableSeats))
    : TOTAL_SEATS;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/reserve", {
        ...
        body: JSON.stringify({ date, times: selectedTimes, seats, name, phone }),
      });
  ...
  return (
    <div className="max-w-xl mx-auto bg-[#1a1a1a] p-8 rounded-xl shadow-2xl border border-gray-800">
      <h1 className="font-heading text-3xl font-bold mb-2 text-center text-purple-500">ggZone Rezervacije</h1>
        <p className="text-gray-400 text-center mb-8">Rezervirajte svoje mjesto za najbolje gaming iskustvo!</p>

        {message && (
          <div className={`p-4 mb-6 rounded ${message.includes("Uspješno") ? "bg-green-600/20 text-green-400 border border-green-500/50" : "bg-red-600/20 text-red-400 border border-red-500/50"}`}>
            {message}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">Datum dolaska</label>
            <input
              type="date"
              required
          ...
          {date && !isLoadingSlots && (
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Dostupni sati (Možete označiti više termina)</label>
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${isFull ? "opacity-30 cursor-not-allowed bg-red-900 border-red-800" :
                        isSelected ? "bg-purple-600 border-purple-500 text-white" : "bg-[#222] border-gray-700 hover:bg-[#333] text-gray-200"}
                      `}
                    >
                      {slot.time}
          ...
          {isLoadingSlots && <p className="text-sm text-gray-400">Provjeravam slobodna mjesta...</p>}

          {selectedTimes.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Koliko računala/konzola?</label>
              <select
                value={seats}
                onChange={(e) => setSeats(Number(e.target.value))}
                className="w-full bg-[#222] border border-gray-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-purple-500 outline-none"
              >
                {Array.from({ length: minAvailableInSelected }, (_, i) => i + 1).map((num) => (
                  <option key={num} value={num}>{num} mjesto(a)</option>
                ))}
              </select>
              <p className="text-xs text-gray-500 mt-2">Maksimalan broj mjesta ovisi o terminu koji ima najmanje slobodnih mjesta.</p>
            </div>
          )}
          ...
          {selectedTimes.length > 0 && (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Ime</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#222] border border-gray-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-purple-500 outline-none"
                />
                <p className="text-xs text-gray-500 mt-2">Mjesto čuvamo određeno vrijeme; nazvat ćemo vas ako kasnite.</p>
              </div>
